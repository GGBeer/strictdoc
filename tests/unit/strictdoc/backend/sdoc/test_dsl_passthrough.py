...
""".lstrip()  # noqa: E501
    ...
    # Bibliography.BibFiles.BibTex => 2 files
    #   ctan_bibtex-test_test.bib  => 85 entries
    #   ctan_biblatex-examples.bib => 92 entries
    # Bibliography.Entries.BibTex => 1 entry
    # Bibliography.Entries.String => 3 entries / 3 citations
    # Bibliography.Entries.Citation => 2 citations
    # Requirement.BibRef.BibTex    => 1 entry / 1 citation
    # Requirement.BibRef.String    => 1 entry (!) / 2 citations
    # Requirement.BibRef.Citation  => 1 citation (duplicate!)

    assert len(doc_bib.bib_files) == 2
    assert len(doc_bib.bib_db.entries) == (85+92+6)

    file_entry = doc_bib.bib_files[0]
    assert file_entry.g_file_path == "bib-files/ctan_bibtex-test_test.bib"
    file_entry = doc_bib.bib_files[1]
    assert file_entry.g_file_path == "bib-files/ctan_biblatex-examples.bib"

    ## Totals
    assert len(doc_bib.bib_entries) == 6
    assert len(doc_bib.bib_citations) == 8
